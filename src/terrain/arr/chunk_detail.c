...
static dvec3 arr2d_dvec3_sample_linear(arr2d_dvec3_t arr, vec2 uv)
{
    ...
    double c = arr2d_dvec3_is_coord_valid(arr, uv_a) * (1.0 - off.x * off.y) +
    arr2d_dvec3_is_coord_valid(arr, ssize2_add(uv_a, (ssize2){1, 0})) * off.x +
    arr2d_dvec3_is_coord_valid(arr, ssize2_add(uv_a, (ssize2){0, 1})) * off.y +
    arr2d_dvec3_is_coord_valid(arr, ssize2_add(uv_a, (ssize2){1, 1})) * off.x * off.y;

    return dvec3_divs(dvec3_add(dvec3_add(
    dvec3_add(dvec3_muls(arr2d_dvec3_sample(arr, uv_a), 1.0 - off.x * off.y),
    dvec3_muls(arr2d_dvec3_sample(arr, ssize2_add(uv_a, (ssize2){1, 0})), off.x)),
    dvec3_muls(arr2d_dvec3_sample(arr, ssize2_add(uv_a, (ssize2){0, 1})), off.y)),
    dvec3_muls(arr2d_dvec3_sample(arr, ssize2_add(uv_a, (ssize2){1, 1})), off.x * off.y)), c);
}
