...
static void serialize_triangle(rtx_triangle *triangle, gpu_serial_t *serial)
{
    for (size_t i = 0; i < 3; i++)
        gpu_serial_write_vec3(serial, triangle->vertex[i]);
    for (size_t i = 0; i < 3; i++)
        gpu_serial_write_vec2(serial, triangle->lightmap.uv[i]);
    gpu_serial_write(serial, 0);
    gpu_serial_write_vec3(serial, triangle->normal);
    gpu_serial_write_vec3(serial, triangle->tangent);
    gpu_serial_write_vec3(serial, triangle->bitangent);
}
